...
def render_painel(crianca_id, crianca_nome: str, primeiro: str) -> None:
    ...
    primeiro_html = html.escape(str(primeiro), quote=True)

    st.markdown(
        f'<div class="page-header">'
        f'  <div class="page-title">Olá, Dr(a). {primeiro_html} 👋</div>'
        f'  <div class="page-subtitle">'
        f'    Acompanhamento clínico de <b>{nome_html}</b>'
    ...
    with col_alert:
        ...
        alertas_html = ''.join(
            ...
            f'<div class="ok-card">'
            f'✅ Nenhum padrão crítico identificado para {nome_curto_html} no período recente.'
            f'</div>'
        )
        st.markdown(
    ...
    with col_dist:
        ...
        for cat, cnt in cat_count.items():
            ...
            cor = html.escape(str(COR_CAT.get(cat, '#6366f1')), quote=True)
            lbl = html.escape(str(LABEL_CAT.get(cat, cat.upper())), quote=True)
            barras += (
    ...
    for _, reg in df.head(20).iterrows():
        ...
        itens += (
            f'<div class="diary-item" style="border-left:3px solid {html.escape(str(cor), quote=True)}">'
            f'  <span style="font-size:.75rem;color:#6b7280;min-width:8.8rem;font-weight:800">'
            f'    {html.escape(str(ts), quote=True)}</span>'
            f'  <span style="font-size:1.3rem;flex-shrink:0">{html.escape(str(emoji), quote=True)}</span>'
            f'  <span style="font-size:.88rem;font-weight:800;color:#1e1b4b;flex:1">{html.escape(str(pict), quote=True)}</span>'
...
def _kpi_html(label: str, valor: str, sub: str, sub_cls: str, accent_cls: str) -> str:
    return (
        f'<div class="kpi-card {html.escape(str(accent_cls), quote=True)}">'
        f'  <div class="kpi-label">{html.escape(str(label), quote=True)}</div>'
        f'  <div class="kpi-value">{html.escape(str(valor), quote=True)}</div>'
        f'  <div class="kpi-sub {html.escape(str(sub_cls), quote=True)}">{html.escape(str(sub), quote=True)}</div>'
        f'</div>'
    )
...
def _calcular_alertas(df, nome: str) -> list:
    ...
    if len(neg) >= 3:
        ...
        alertas.append(
            f"🌀 <b>{top[1]}×</b> a emoção <b>{html.escape(str(top[0]).lower(), quote=True)}</b> nos últimos 7 dias — avaliar estado emocional."
        )
    noturno = int(((df['hora'] >= 23) | (df['hora'] <= 5)).sum())
    if noturno >= 3:
        alertas.append(
            f"🌙 <b>{noturno} interações noturnas</b> (23h–5h) — investigar possível alteração do padrão de sono."
        )
    if not df.empty:
    ...
    if ajuda >= 3:
        alertas.append(
            f"🆘 <b>\"AJUDA\"</b> selecionada <b>{ajuda}×</b> — verificar necessidade de suporte adicional."
        )
    return alertas

...
def render_hoje(criancas, crianca_id, crianca_nome, primeiro_nome: str) -> None:
    ...
    primeiro_html = html.escape(str(primeiro_nome), quote=True)
    st.markdown(
        f'<div class="page-header">'
        f'  <div class="page-title">Olá, {primeiro_html}! 👋</div>'
        f'  <div class="page-subtitle">Aqui está o que <b>{nome_curto_html or "sua criança"}</b>'
    ...
    if not hoje_ints:
        st.markdown(
            ...
            f'    {nome_curto_html} ainda não usou o PICTA hoje</div>'
            f'  <div style="color:#9ca3af;font-weight:600">'
            f'    Quando {nome_curto_html} se comunicar, tudo aparece aqui.</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
        _banner_total(interacoes, nome_curto_html)
        return
    ...
    if em_top:
        ...
        st.markdown(
            f'<div class="wb-hero">'
            f'  <span class="wb-emoji">{html.escape(str(em_emoji), quote=True)}</span>'
            f'  <div class="wb-phrase">Hoje {nome_curto_html} {html.escape(str(frase), quote=True)}</div>'
    ...
    if necs or acoes:
        ...
        with col_n:
            if necs:
                st.markdown('<div class="sec-header">🙋 O que pediu hoje</div>', unsafe_allow_html=True)
                for pict, qtd in Counter(i['pictograma'] for i in necs).most_common():
                    em = next((i['emoji'] for i in necs if i['pictograma'] == pict), '')
                    _diary_item(em, pict.title(), f"{qtd}× solicitado", 'badge-necessidade', 'Pedido')
        with col_a:
            if acoes:
                st.markdown('<div class="sec-header">🎯 O que fez hoje</div>', unsafe_allow_html=True)
                for pict, qtd in Counter(i['pictograma'] for i in acoes).most_common():
                    em = next((i['emoji'] for i in acoes if i['pictograma'] == pict), '')
                    _diary_item(em, pict.title(), f"{qtd}× realizado", 'badge-acao', 'Ação')

    dicas = []
    if necs:
        top = Counter(i['pictograma'] for i in necs).most_common(1)[0][0].lower()
        dicas.append(f"{nome_curto_html} pediu <b>{html.escape(str(top), quote=True)}</b> hoje. Que tal oferecer à tarde?")
    if [i for i in emocoes if i['pictograma'] in EMOCOES_ATENCAO]:
        dicas.append(f"{nome_curto_html} demonstrou emoções de atenção hoje. Uma atividade calma pode ajudar.")
    if acoes:
        top_a = Counter(i['pictograma'] for i in acoes).most_common(1)[0][0].lower()
        dicas.append(f"{nome_curto_html} quis <b>{html.escape(str(top_a), quote=True)}</b> hoje — ótimo sinal de engajamento!")
    if dicas:
...
def _banner_total(interacoes: list, nome_curto_html: str) -> None:
    if interacoes:
        st.markdown(
            ...
            f'No total, {nome_curto_html} fez '
...
def _mini_stat(col, valor: str, desc: str, icon: str) -> None:
    with col:
        st.markdown(
            ...
            f'  <div class="mini-stat-icon">{html.escape(str(icon), quote=True)}</div>'
            f'  <div class="mini-stat-value">{html.escape(str(valor), quote=True)}</div>'
            f'  <div class="mini-stat-label">{html.escape(str(desc), quote=True)}</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
def _diary_item(emoji: str, nome: str, tempo: str,
    ...
    st.markdown(
        ...
        f'  <div><div class="diary-name">{html.escape(str(nome), quote=True)}</div>'
        f'  <div class="diary-time">{html.escape(str(tempo), quote=True)}</div></div>'

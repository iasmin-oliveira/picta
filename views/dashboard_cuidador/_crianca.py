...
def render_crianca(usuario_id, criancas, crianca_id, crianca_nome: str) -> None:
    st.markdown(
        ...
        '  <div class="page-subtitle">Visualize e edite os dados da criança, e veja dicas personalizadas.</div>'
    ...
    if not criancas or not crianca_id:
        st.markdown(
            ...
            '  <div style="color:#9ca3af;font-weight:600;margin-top:.4rem">Acesse <b>Vínculos</b> para adicionar uma criança.</div>'
            '</div>', unsafe_allow_html=True,
        )
        return
    ...
    crianca_db = obter_crianca_por_id(crianca_id, usuario_id=usuario_id, perfil=perfil)
    if not crianca_db:
        st.error("Criança não encontrada ou sem permissão de acesso.")
        return
    ...
    with col_info:
        dn = crianca_db.get('data_nascimento')
        ...
        hoje_ints = [i for i in interacoes if str(i.get('registado_em', ''))[:10] == hoje]
        ...
        st.markdown(
            f'<div class="glass-card" style="padding:1rem 1.2rem;"><div class="sec-header" style="margin-bottom:.6rem">📊 Resumo geral</div>'
            f'<div class="ficha-row"><span class="ficha-label">Total de comunicações</span><span class="ficha-valor">{len(interacoes)}</span></div>'
            f'<div class="ficha-row"><span class="ficha-label">Comunicações hoje</span><span class="ficha-valor">{len(hoje_ints)}</span></div>'
            f'<div class="ficha-row"><span class="ficha-label">Pedidos registrados</span><span class="ficha-valor">{len(necs_all)}</span></div>'
            f'<div class="ficha-row"><span class="ficha-label">Emoções registradas</span><span class="ficha-valor">{len(emocs_all)}</span></div></div>', unsafe_allow_html=True,
        )
    ...
    with col_edit:
        st.markdown('<div class="sec-header">✏️ Editar dados da criança</div>', unsafe_allow_html=True)
        nome_atual = crianca_db.get('nome', crianca_nome)
        dn_atual = dn_str
        with st.form("form_editar_crianca"):
            novo_nome = st.text_input("Nome da criança", value=nome_atual)
            nova_dn = st.text_input("Data de nascimento", value=dn_atual, placeholder="AAAA-MM-DD  (ex: 2018-05-10)", help="Formato: Ano-Mês-Dia")
            ...
            if submitted:
                ...
                if dn_val:
                    try:
                        ...
                        st.stop()
                erro = atualizar_crianca(crianca_id, novo_nome, dn_val, usuario_id=usuario_id, perfil=perfil)
                if erro:
                    st.error(f"❌ {erro}")
                else:

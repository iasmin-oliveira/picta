...
def render():
    ...
    with col_hdr:
        ...
        <div class="dash-header">
            <div>
                <h2>📊 Dashboard — Olá, {nome.split()[0]}!</h2>
                <p>👩‍⚕️ Perfil Cuidador / Terapeuta · Área Restrita</p>
            </div>
    ...
    st.divider()
    st.markdown("#### 📋 Últimas interações registadas")
    ...
    if interacoes:
        for reg in interacoes[:10]:
            ...
            st.markdown(
                f"`{ts}` → {reg.get('emoji','')} **{reg.get('pictograma','')}** "
                f"<span style='background:{cor[\"bg\"]};padding:2px 8px;border-radius:8px;"
                f"font-size:0.78rem;color:{cor[\"text\"]};font-weight:700'>{cat.upper()}</span>",
                unsafe_allow_html=True
            )
    else:
    ...
    <div class="coming-soon">
        ...
        <h4 style="margin:0.5rem 0">Gráficos analíticos — Ciclo 3</h4>
        <p style="color:#8B7EA8;font-size:0.9rem;margin:0">
            Gráficos Pandas e exportação CSV/PDF serão implementados no Ciclo 3 (16/05–22/06).
        </p>
    </div>
    """, unsafe_allow_html=True)
